...
contract ERC223Token is IERC223 {
    ...
    // Burn Function ---> Input the amount of tokens need to be burnt
    function burn(uint _value) public override returns (bool success) {
    ...
    // Input the address of the owner
    function burnFrom(address _from, uint _value) public override returns (bool success) {
    ...
    // Mint Token ---> Input the address and amount to be minted (Only owner has access to mint)
    function mint(address target, uint mintAmount) onlyOwner {
        balanceOf[_target] += mintAmount;
